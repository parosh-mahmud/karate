...
export default function RunningRegistrationsPage() {
  ...
  const handleUpdateStatus = async (registration) => {
    ...
    try {
      ...
      // 2. If confirming, validate data BEFORE sending the email
      if (newStatus === "confirmed") {
        // ** ADD THIS VALIDATION BLOCK **
        if (!registration.name || !registration.email) {
          console.error(
            "Cannot send email: Name or email is missing for this registration.",
            registration
          );
          alert(
        ...
        if (!response.ok) {
          console.warn("Confirmation email may have failed to send.");
        }
      }
    } catch (err) {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `running_seminar_registrations_${new Date().toISOString().split("T")[0]}.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
                ...
                <tr key={reg.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    ...
                    <button
                      ...
                      disabled={updatingId === reg.id}
                      className={`px-3 py-1 rounded-md text-white font-semibold transition duration-300 w-36 text-center ${updatingId === reg.id ? "bg-gray-400 cursor-not-allowed" : (reg.status || "pending") === "confirmed" ? "bg-yellow-500 hover:bg-yellow-600" : "bg-green-500 hover:bg-green-600"}`}
                    >
                      {updatingId === reg.id
                        ? "Updating..."
                        : (reg.status || "pending") === "confirmed"
